        ...
        >
          ...
          >
            <template slot-scope="scope">
              {{scope.row.orderStatus == 6 ? "已完成":"未完成"}}
            </template>
          </el-table-column>
          <el-table-column
            prop="orderNo"
...
export default {
  ...
  methods: {
    ...
    initData () {
      ...
      let queryPage = JSON.parse(JSON.stringify(this.queryPage))

      queryPage['productTypeSubId'] = this.$route.params.productTypeSubId

      statistics.queryCusOrderPage(queryPage).then(res => {
        if (res.code === 0) {

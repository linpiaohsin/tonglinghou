<template>
  <div class="productType">
    ...
    <div class="main">
      <div class="table">
        <router-link :to="{name:'AddSubProductType'}" class="btn" v-if="isBtnShow.addBtn">添加</router-link>
        <el-table :data="tableData" border style="width: 1200px">
          <el-table-column type="index" label="序号" width="50px">
          </el-table-column>
          <el-table-column prop="name" label="类型名称" width="220px">
          </el-table-column>
          <el-table-column prop="pName" label="一级类型名称" width="100px">
          </el-table-column>
          ...
          <el-table-column fixed="right" label="操作" width="100">
            <template slot-scope="scope">
              <router-link :to="{name:'EditSubProductType',query: {id: scope.row.id}}" v-if="isBtnShow.editBtn">编辑</router-link>
  ...
  methods: {
    ...
    handleClick (row) {
      this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
        ...
        priceSystem.productTypeSubDelete(row.id).then(res => {
          if (res.code == 0) {
    ...
    // 初始化数据
    initData () {
      priceSystem.productTypeSubQuery(this.queryPage).then(res => {
        if (res.code == 0) {
